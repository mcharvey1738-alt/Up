...
-- Retry up to 10s if Networking not ready yet (timing fix for loader)
if not Net then
    for _ = 1, 10 do
        task.wait(1)
        local ok, m = pcall(function() return require(ReplicatedStorage.SharedModules.Networking) end)
        if ok and m then Net = m; break end
    end
end
if not Net then warn("[Belle.sg] GAG2 - Networking module missing, aborting."); return end
...
local function stealTargets()
    ...
    for _, plot in ipairs(gardens:GetChildren()) do
        local ownerUid = tonumber(plot:GetAttribute("OwnerUserId"))
            ...
            for _, obj in ipairs(plot:GetDescendants()) do
                if obj:IsA("ProximityPrompt") then
                    ...
                    if part and part:IsA("BasePart") then
                        ...
                        if model and model:GetAttribute("PlantId") and not seen[model] then
                            seen[model] = true
                            local uid = tonumber(model:GetAttribute("UserId")) or ownerUid
                            if uid ~= LocalPlayer.UserId then
                                out[#out + 1] = { model = model, value = fruitValue(model) }
                            end
                        end
                    end
                end
    ...
    -- fallback: also check CollectionService tags in case game uses both
    for _, p in ipairs(CollectionService:GetTagged("StealPrompt")) do
        ...
        if m and not seen[m] then
            ...
            if uid and uid ~= LocalPlayer.UserId and m:GetAttribute("PlantId") then
                out[#out + 1] = { model = m, value = fruitValue(m) }
            end
        end
    end
...
genv.BELLESG_notify = function(msg, title) notify(msg, title) end
